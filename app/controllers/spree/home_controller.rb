module Spree
  class HomeController < Spree::StoreController
    ...
    def ranking
      @ranking = Spree::Product.find_by_sql("select * from spree_products where id in (select c.product_id from spree_orders a left join spree_line_items b on a.id = b.order_id left join spree_variants c on b.variant_id = c.id group by c.product_id order by sum(b.quantity) desc) limit 20")
      @taxonomies = Spree::Taxonomy.includes(root: :children)
    end
  end
end
